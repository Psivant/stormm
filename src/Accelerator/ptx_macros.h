...
// Transfer memory between two arrays without caching in either the read or write operations:
#  define NON_CACHING_XFER(dest, orig, dest_idx, orig_idx) { \
  __stwt(&dest[dest_idx], __ldcv(&orig[orig_idx])); \
}
